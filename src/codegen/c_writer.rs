...
impl CWriter {
    ...
    pub fn start_decl_func(&mut self, ret_type: &NodeType, name: &str, params: &[(NodeType, String)]) {
        let param_str: Vec<String> = params.iter().map(|(param_type, name)| {
            format!("{} {}", Self::convert_type(param_type), name)
        }).collect();
        let param_str = param_str.join(",");
        self.writeln(&format!("{} {}({}) {{", Self::convert_type(ret_type), name, param_str));
        self.indent += 1;
    }
    ...
    fn indent<F>(&mut self, block: F) where F: Fn(&mut CWriter) -> () {
        self.indent += 1;
        block(self);
        self.indent -= 1;
    }
